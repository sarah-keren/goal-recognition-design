...
class DomainDef(Visitable):
    ...
    def __init__(self, name, requirements=None, types=None, predicates=None,
                 actions=None, constants=None, actionCosts = False):
...
def _parse_type_helper(iter, type_class):
    ...
    while not iter.empty():
        ...
        if type_class != Variable and len(var) > 0 and var[0] in reserved:
            ...
            if type_class == Variable:
                #TODO:SARAH - use this for debugging
                #print('var is :')
                #print(var)
                if var[0] != '?':
...
def parse_domain_def(iter):
    ...
    while not iter.empty():
        ...
        #SARAH : supporting arbitrary actions costs
        elif key.name == 'functions':
            domain.actionCosts = True
        elif key.name == 'types':
            ...
            #TODO SARAH : Adding equality predicates here 
            for keyword in domain.requirements.keywords:
                if keyword.name == 'equality': 
                    #type = domain.types [0]
                    parameters = list()
                    parameters.append(Variable('a')) 
                    parameters.append(Variable('b'))                                        
                    equality_predicate = Predicate('=',parameters)
                    pred.predicates.append(equality_predicate)                              
...
def parse_problem_def(iter):
    ...
    # skip the requirements, if present
    if iter.peek_tag() == ':requirements':
        next(iter)         
...
def parse_init_stmt(iter):
    ...
    if not iter.try_match(':init'):
        raise ValueError('Error found invalid keyword {} when parsing InitStmt'.format(next(iter)))
...
class Parser(object):
    ...
    def parse_domain(self, read_from_file=True):
        ...
        if '=' in visitor.domain.predicates :
            #TODO SARAH : Remove the equality predicate
            del visitor.domain.predicates['='] 
