...
#TODO SARAH - the flag IsNot was added to allow for 'not' in preconditions
class Predicate:
    def __init__(self, name, signature,IsNot = False):
        """
...
class Action:
    def __init__(self, name, signature, precondition, effect):
        ...
        for pred in self.effect.addlist:
            if 'increase' in pred.name and len(pred.signature) != 0:
                self.cost = (int)(pred.signature[1][0])



class Domain:
    def __init__(self, name, types, predicates, actions, constants={}, actionCosts = False, requirements = {}):
        """
