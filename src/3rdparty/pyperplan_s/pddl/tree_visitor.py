...
class TraversePDDLDomain(PDDLVisitor):
    ...
    def visit_domain_def(self, node):
        ...
        self.domain = pddl.Domain(node.name, self._types, self._predicates,
                                  self._actions, self._constants, self._actionCosts, self._requirements)
    ...
    def add_precond(self, precond, c):
        ...
        if(c.key != 'not'):
            ...
            if len(c.children[0].children) != len(predDef.signature):
                raise SemanticError('Error: wrong number of arguments for '
                                    'predicate ' + c.children[0].key + ' in precondition of '
                                    'action')
    ...
    def visit_precondition_stmt(self, node):
        ...
        if formula.key == 'and':
            ...
            for c in formula.children:
                ...
                #TODO SARAH - added by Sarah to comply with not predicates
                if c.key == 'not':
                    if not c.children[0].key in self._predicates:
                            print("TODO:SARAH Before error key in not and predicates are ")
                            print(c.key)                    
                            print(c)
                            print(self._predicates)    
                            raise SemanticError('Error unknown predicate ' + c.children[0].key +
                                            ' used in precondition of action with not ')
                    
                else :
                        
                    if not c.key in self._predicates:                        
                        #TODO:SARAH delete this 
                        print("TODO:SARAH Before error key and predicates are ")
                        print(c.key)                    
                        print(c)
                        print(self._predicates)
    ...
    def add_effect(self, effect, c):
        ...
        count = 0
        #SARAH : supporting arbitrary actions costs
        if not 'increase' in nextPredicate.key:
